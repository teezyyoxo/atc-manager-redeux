...
export const loadMap = key => {
  if (!maps[key]) throw new Error(`Unknown map: ${key}`);
  const map = JSON.parse(JSON.stringify(maps[key]));
  Object.keys(map.waypoints)
    .map(k => map.waypoints[k])
    .forEach(w => (w.type = idType.WAYPOINT));
  map.airport.type = idType.AIRPORT;
  map.airport.runways.forEach(rwy => (rwy.type = idType.RWY));
  return map;
};
...
export const activeRwys = (airport, winddir) => {
  ...
  if (ru) {
    ...
    for (let i = 0; i < ru.length; i++) {
      if (winddir < ru[i].dir) {
        const runwayUsage = ru[(i + ru.length - 1) % ru.length];
        return runwayUsage.rwys;
      }
    }
    ...
    for (let i = 0; i < airport.runways.length; i++) {
      ...
      aRwys.push(
        Math.abs(angleDelta(winddir, rwy.hdg1)) <
        Math.abs(angleDelta(winddir, rwy.hdg2))
          ? rwy.name1
          : rwy.name2
      );
    }
...
export const mapsArr = Object.values(maps).sort((left, right) =>
