...
export const getBuildInfo = () => {
  const version = getMetaContent('application-version') || '3.0.0-rc.4';
...
export const getChangelog = () => {
  const raw = getMetaContent('application-changelog');
  if (!raw) {
    const latest = getReleaseNotes();
    return latest ? [latest] : [];
  }
  try {
    const releases = JSON.parse(decodeURIComponent(raw));
    return Array.isArray(releases) ? releases : [];
  } catch (error) {
    console.warn('Unable to read the changelog for this build.', error);
    return [];
  }
};

export const shouldAnnounceBuild = (lastSeenBuild, build, notes) =>
  !!notes &&
  notes.version === build.version &&
  lastSeenBuild !== build.label;
