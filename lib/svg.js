...
.plane-path.ground-track-vector {
  filter: drop-shadow(0 0 3px ${settingsObj.pathVisualizerColor});
...
.map-view-street .background path,
.map-view-terrain .background path,
.map-view-satellite .background path {
  fill: none;
  opacity: 0.48;
  stroke: ${settingsObj.foregroundColor};
  vector-effect: non-scaling-stroke;
}
.geographic-map-attribution rect {
  fill: rgba(5, 15, 19, .78);
  stroke: rgba(255, 255, 255, .28);
}
.geographic-map-attribution text {
  fill: #fff;
  font: 11px sans-serif;
}
.geographic-map-attribution:hover text,
.geographic-map-attribution:focus text {
  text-decoration: underline;
}
.Airport circle {
  stroke: ${settingsObj.foregroundColor}
