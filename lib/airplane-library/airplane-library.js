...
import { longestRunwayLen } from '../map';
import { modelAllowedForOperatorAtRegion } from './current-fleets';
...
export const rndModelWithEnoughRwyLen = (mapAirport, ga) => {
  ...
  const aps = airplanes.filter(ap =>
    ...
    && (ga || ap.operators.some(operator =>
      operatorAllowedAtAirport(operator, mapAirport, ap.id)
    )));
  let counter = 0;
  const airplaneRatios = aps.map(plane => ({
    airplane: plane,
...
const operatorAllowedAtAirport = (operatorId, airport, modelId) => {
  if (
    Array.isArray(airport.operatorWhitelist) &&
    !airport.operatorWhitelist.includes(operatorId)
  ) return false;

  const operator = operatorsById[operatorId];
  if (!operator) return false;
  if (!airport.region || airport.region === 'global') return true;
  if (!Array.isArray(operator.regions) || operator.regions.length === 0) {
    return true;
  }
  const servesRegion = operator.regions.includes(airport.region) ||
    operator.regions.includes('cargo-global');
  return servesRegion && modelAllowedForOperatorAtRegion(
    modelId,
    operator,
    airport.region
  );
...
export const rndOperator = (airplane, airport) => {
  const allowed = airplane.operators.filter(op =>
    operatorAllowedAtAirport(op, airport, airplane.id)
  );
  const operators = allowed.map(op => operatorsById[op]).filter(Boolean);
