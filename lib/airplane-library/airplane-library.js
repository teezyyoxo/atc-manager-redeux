...
export const rndModelWithEnoughRwyLen = (mapAirport, ga) => {
  ...
  const aps = airplanes.filter(ap =>
    ...
    && (
      ga ||
      !Array.isArray(mapAirport.operatorWhitelist) ||
      ap.operators.some(operator =>
        mapAirport.operatorWhitelist.includes(operator)
      )
    ));
  let counter = 0;
  const airplaneRatios = aps.map(plane => ({
...
export const rndOperator = (airplane, allowedOperatorIds) => {
  const allowed = Array.isArray(allowedOperatorIds)
    ? airplane.operators.filter(op => allowedOperatorIds.includes(op))
    : airplane.operators;
