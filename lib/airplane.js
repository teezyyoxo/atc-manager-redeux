...
for (let i = flightNums.length - 1; i > 0; i--) {
  const j = Math.floor(Math.random() * (i + 1));
  [flightNums[i], flightNums[j]] = [flightNums[j], flightNums[i]];
}
...
const nextRegNum = () => {
  const registration = regNums[regNumCounter++];
  regNumCounter %= regNums.length;
  return registration;
};
...
const createAirplane = (x, y, heading, routeType, model, ga, map) => {
  let operatorId = model.operators.length > 0
    ? rndOperator(model, map.airport.operatorWhitelist)
    : null;
  ...
  if (operatorId === null) airplane.regNum = nextRegNum();
  return airplane;
};
...
const headingFromMiddle = (x, y) => {
  ...
  return Math.floor(hdgFrom + 360) % 360;
};
...
export default class Airplane {
  ...
  static isVFR(airplane) {
    const routeType = airplane && routeTypes[airplane.routeType];
    return (
      airplane &&
      airplane.tgtVfrState !== undefined &&
      typeof routeType === 'string' &&
      routeType.startsWith('vfr')
    );
  }
}
