...
export class TakeoffRunwayAssignment {
  constructor(map, callsignsPos, disableTakoffsOnRwysSet, windspd, winddir) {
    this.getMeta = (model, ga) => { // only runs once, the return value is cached
      ...
      const activeRunwaysWithinWindLimits = activeRunways.filter(rwy => {
        const cs = callsignsPos[rwy].ref;
        const heading = cs.name1 === rwy ? cs.hdg1 : cs.hdg2;
        const wind = runwayWindComponents(heading, winddir, windspd);
        return wind.crosswind <= (model.maxCrosswind || 25) &&
          wind.tailwind <= (model.maxTailwind || 10);
      });
      const couldNotFindRunwayWithinWindLimits =
        activeRunwaysWithinWindLimits.length === 0;
      if (couldNotFindRunwayWithinWindLimits === false) {
        activeRunways = activeRunwaysWithinWindLimits;
      }
      ...
      const ret = {
        ...
        couldNotFindRunwayWithinWindLimits,
