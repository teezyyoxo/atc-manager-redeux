...
import { angleDelta, parrelelLinesDistance, headingTo } from './map';
...
const legScore = (airplane, leg, callsignPositions) => {
  ...
  if (!from || !to || typeof leg.to.dir === 'number') return Infinity;
  ...
  return (
    ...
    Math.abs(angleDelta(airplane.heading, legHdg)) / 10
  );
...
export const mostSuitableLeg = (airplane, sidOrStar, callsignPositions) => {
  ...
  if (!best) return { complete: true, best: null };
  while (
    callsignPositions[best.to.dir] &&
    ...
    if (typeof best.to.dir === 'number') return { complete: false, best };
  }
