...
all be overridden. The default image is `atc-manager:3.0.0-rc.4`.
...
`3.0.0-rc.4+<commit>`, which identifies the exact release-candidate build in
use.
...
```bash
podman build --format docker --build-arg APP_VERSION=3.0.0-rc.4 \
  -t localhost/atc-manager:3.0.0-rc.4 .
podman run --pull=never --rm -p 8080:80 localhost/atc-manager:3.0.0-rc.4
```
...
```bash
docker run --rm -p 8081:80 atc-manager:3.0.0-rc.4
podman run --rm -p 8081:80 localhost/atc-manager:3.0.0-rc.4
```
